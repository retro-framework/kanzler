...
async function getAffix(hash: HashStr): Promise<IAffix> {
    ...
    const getEvent = async (eventHash: HashStr): Promise<IEvent> => {
        const getEventResult = await fetch(`/obj/${eventHash}`).then(response => response.json()).catch(e => console.error);
...
function mapDispatchToProps(dispatch: Dispatch<actions.ISelectedODBVCheckpointChanged|actions.IODBVAffixAvailable>): IPropsFromDispatch {
    return {
